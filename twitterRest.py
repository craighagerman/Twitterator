...
class TwitterRestClient(application_only_auth.Client):
    ...
    def _queryRestApi(self, resource, parameters):
        query = "{}/{}/{}?{}".format(self.API_ENDPOINT, self.API_VERSION, resource, self._concatParam(parameters) )
    ...
    # public methods
    def get_single_user(self, screen_name):                         # GET users/show rate limit: 180 / 15 min
        '''  (Previously get_profile) Return the profile metadata associated with a single user's screen_name  '''
        resource = "users/show.json"
        parameters = {"screen_name": screen_name}
        return self._queryRestApi(resource, parameters)
    
    def get_user_timeline(self, screen_name, max_id=None):     # GET statuses/user_timeline rate limit: 300 / 15 min
        resource = "statuses/user_timeline"
        return self._get_statuses(screen_name, resource, max_id)

    def get_home_timeline(self, screen_name, max_id=None):       # GET statuses/user_timeline rate limit: 300 / 15 min
        resource = "statuses/user_timeline"
        resource = "statuses/user_timeline"
        return self._get_statuses(screen_name, resource, max_id)
    ...
    def get_friends(self, screen_name, cursor="-1", skip_status="false", include_user_entities="false"):                  #  GET friends/list rate limit: 30 / 15 min
        resource = "friends/list.json"
        return self._get_community(screen_name, resource, cursor, skip_status, include_user_entities)

    def get_followers(self, screen_name, cursor="-1", skip_status="false", include_user_entities="false"):                  #  GET followers/list rate limit: 30 / 15 min
    ...
    def get_users_lookup(self, screen_name_list):
        '''  Return the profile metatdata associated with a list of screen_names (max len = 100)'''
        resource = "users/lookup.json"
        parameters = {"screen_name": ",".join(screen_name_list)}
        return self._queryRestApi(resource, parameters)

    def get_statuses_lookup(self, tweet_id_list):                        # GET statuses/lookup rate limit: 60 / 15 min
...
def makeTSV(verifiedFile, outputTSVfile):
    ...
    with open(outputTSVfile, "w") as fo:
        for line in vlines:
            d = json.loads(line)
            tsv = "\t".join(list(map(lambda x: re.sub(nlpattern, " ", str(x)), [ d['id_str'], d['screen_name'], d['name'], d['lang'], d['description'], d['statuses_count'], d['friends_count'], d['followers_count'], d['listed_count'], d['location'], d['time_zone'], d['url'] ])))
            fo.write(tsv + "\n")
...
def queryUserList(api, userlist, outfile):
    ...
    with open(outfile, "a") as fo:
        for user in userlist:
            ...
            userdetails = [ u['id_str'], u['screen_name'], u['name'], str(u['statuses_count']), u['location'], u['time_zone'] ]
